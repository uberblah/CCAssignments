...
def lispexpr(n):
	...
	def lispexpr_rec(n):
		...
		if isinstance(n,list) or isinstance(n,tuple):
			...
			return ['return',le(n.value)]
		elif isinstance(n, Function):
			return ['=',n.name,['lambda',n.argnames,le(n.code)]]
			...
			return ['while'] + map(le,n.getChildren())
		elif isinstance(n, Getattr):
			return ['call', 'get_attr'] + map(le, n.getChildren())
		elif isinstance(n, Class):
			return ['class', le(n.name), le(n.bases), le(n.code)]
		else:
			raise Exception('Unrecognized AST Node: ' + str(n))
